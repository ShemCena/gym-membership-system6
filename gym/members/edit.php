...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existingMember = $member->getByEmail($data['email']);
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
        $uploadResult = uploadPhoto($_FILES['photo'], $memberData['photo']);
        ...
        // Keep existing photo
        $data['photo'] = $memberData['photo'];
    }
    
    if (empty($errors)) {
        if ($member->update($id, $data)) {
            redirect('index.php', 'Member updated successfully!', 'success');
        } else {
...
<head>
    ...
    <title>Edit Member - Fitness Club Management System</title>
...
<body class="bg-slate-900 text-white">
    ...
    <div class="ml-56 p-4">
        ...
        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold mb-1">Edit Member</h1>
                <p class="text-gray-400 text-sm">Update member information</p>
            </div>
        ...
        <div class="glass-card p-4">
            <form method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="file-input-wrapper border-2 border-dashed border-gray-600 rounded-lg p-4 text-center transition-all cursor-pointer" onclick="document.getElementById('photo').click()">
                            <div id="photoPreview" class="mb-3">
                                <?php if ($memberData['photo']): ?>
                                    <img src="../uploads/members/<?php echo htmlspecialchars($memberData['photo']); ?>" alt="Current Photo" class="w-24 h-24 object-cover rounded-lg mx-auto photo-preview">
                                <?php else: ?>
                                    <i class="fas fa-camera text-3xl text-gray-500"></i>
                                <?php endif; ?>
                            </div>
                            <p class="text-gray-400 text-sm">Click to change photo</p>
                    ...
                    <div class="space-y-3">
                        <div>
                            ...
                            <input 
                                type="text" 
                                name="full_name" 
                                value="<?php echo htmlspecialchars($memberData['full_name']); ?>"
                        ...
                        <div>
                            ...
                            <input 
                                type="email" 
                                name="email" 
                                value="<?php echo htmlspecialchars($_POST['email'] ?? $memberData['email']); ?>"
                                class="w-full px-3 py-2 bg-slate-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 form-input text-sm"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input 
                            type="tel" 
                            name="phone" 
                            value="<?php echo htmlspecialchars($_POST['phone'] ?? $memberData['phone']); ?>"
                            class="w-full px-3 py-2 bg-slate-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 form-input text-sm"
                    ...
                    <div>
                        ...
                        <input 
                            type="text" 
                            name="address" 
                            value="<?php echo htmlspecialchars($_POST['address'] ?? $memberData['address']); ?>"
                            class="w-full px-3 py-2 bg-slate-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500 form-input text-sm"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="member_type" class="w-full px-3 py-2 bg-slate-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-indigo-500 form-input text-sm" required>
                            <option value="Regular" <?php echo ($_POST['member_type'] ?? $memberData['member_type']) === 'Regular' ? 'selected' : ''; ?>>Regular</option>
                            <option value="Student" <?php echo ($_POST['member_type'] ?? $memberData['member_type']) === 'Student' ? 'selected' : ''; ?>>Student</option>
                            <option value="Senior" <?php echo ($_POST['member_type'] ?? $memberData['member_type']) === 'Senior' ? 'selected' : ''; ?>>Senior</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input 
                            type="date" 
                            name="join_date" 
                            value="<?php echo htmlspecialchars($_POST['join_date'] ?? $memberData['join_date']); ?>"
                    ...
                    <div>
                        ...
                        <input 
                            type="date" 
                            name="expiry_date" 
                            value="<?php echo htmlspecialchars($_POST['expiry_date'] ?? $memberData['expiry_date']); ?>"
                            class="w-full px-3 py-2 bg-slate-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-indigo-500 form-input text-sm"
                            required
                        >
                    </div>
                ...
                <div>
                    ...
                    <div class="flex space-x-3">
                        <label class="flex items-center">
                            <input type="radio" name="status" value="Active" <?php echo ($_POST['status'] ?? $memberData['status']) === 'Active' ? 'checked' : ''; ?> class="mr-2" required>
                            <span class="text-green-400 text-sm">Active</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="status" value="Expired" <?php echo ($_POST['status'] ?? $memberData['status']) === 'Expired' ? 'checked' : ''; ?> class="mr-2">
                ...
                <div class="bg-slate-800/50 rounded-lg p-3">
                    <h3 class="text-base font-semibold mb-2 text-sm">Member Information</h3>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-xs"><?php echo date('M d, Y', strtotime($memberData['created_at'])); ?></p>
                        </div>
                        <div>
                            <span class="text-gray-400">Days Until Expiry:</span>
                            <p class="font-semibold text-xs"><?php 
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-xs"><?php echo htmlspecialchars($memberData['status']); ?></p>
                        </div>
                    </div>
                </div>
        ...
        function previewPhoto(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    preview.innerHTML = `<img src="${e.target.result}" alt="Preview" class="w-32 h-32 object-cover rounded-lg mx-auto photo-preview">`;
                };
